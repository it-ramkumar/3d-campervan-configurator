...
export default function AdminForms() {
  ...
  const fetchCategories = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_URL}/exterior/category`);
  ...
  const fetchSubCategories = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_URL}/exterior/subcategory`);
  ...
  useEffect(() => {
    if (editData) {
      setInteriorForm({
        ...
        // Agar category object hai toh uski _id nikalien, warna direct value
        categoryId: editData.categoryId?._id || editData.categoryId || "",
        subCategoryId: editData.subCategoryId?._id || editData.subCategoryId || "",
        // Backend se description array aa raha hai, handle empty case
        descriptions: editData.description && editData.description.length > 0
          ? editData.description
          : [""],
        // Images purani wali show karne ke liye (Sirf previews ke liye ya references ke liye)
  ...
  const handleInteriorSubCategorySubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_URL}/exterior/subcategory`, {
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      {activeTab === "interior" && (
        <form onSubmit={handleExteriorSubmit} className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
          <h2 className="text-xl font-semibold text-gray-800 mb-6 pb-2 border-b border-gray-100">
            {editData && editData._id ? "Edit Exterior Choice" : "Create New Exterior Choice"}
          </h2>
          ...
          <div className="mb-4">
            ...
            <input
              type="text"
              placeholder="Enter exterior choice title"
          ...
          <div className="mb-4">
            ...
            <input
              type="text"
              placeholder="Enter exterior choice link"
          ...
          <button
            ...
            disabled={loading}
          >
            {editData && editData._id ? "Update Exterior Choice" : "Create Exterior Choice"}
          </button>
        </form>
      )}
    </div>
