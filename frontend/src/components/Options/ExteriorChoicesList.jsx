...
export default function ExteriorChoicesList({ initialData, heading }) {
  // --- FIXED: Properly initialize all states with fallbacks ---
  ...
  const [showFullItemListMap, setShowFullItemListMap] = useState(initialData?.showFullItemListMap || {});

  const [searchQuery, setSearchQuery] = useState("");
  const [selectedCategoryFilter, setSelectedCategoryFilter] = useState("all");

  // --- FIXED: Sync server data to client state when initialData updates ---
  useEffect(() => {
    if (initialData) {
      setActiveSubCategoryMap(initialData.activeSubCategoryMap || {});
      setActiveItemMap(initialData.activeItemMap || {});
      setExpandedCategories(initialData.expandedCategories || {});
      setShowFullItemListMap(initialData.showFullItemListMap || {});
    }
  }, [initialData]);
  ...
  // --- Filtered Categories Logic (Wrapped in useMemo for performance) ---
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-10">
        <AnimatePresence mode="popLayout">
          {filteredCategories?.map((cat, index) => {
            // --- FIXED: Safe map access with optional chaining ---
            ...
            const isShowingFull = showFullItemListMap?.[cat._id] || false;
                ...
                >
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <RichParagraph className="text-primary/40 mt-1 uppercase">
                  ...
                  {isExpanded && (
                    ...
                    >
                        ...
                        {cat.subCategories?.length > 0 && (
                          <div className="px-8 pb-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                            {cat.subCategories.map((sub) => {
                              ...
                              return (
                                <SecondaryButton
                                  key={sub._id}
                                  onClick={() => {
                                    setActiveSubCategoryMap(prev => ({ ...prev, [cat._id]: sub._id }));
                                    setActiveItemMap(prev => ({ ...prev, [cat._id]: sub.items?.[0] || null }));
                                  }}
                                  className={`text-left p-5 rounded-lg border-2 transition-all duration-300 group
                                    ${isActive ? "bg-primary border-primary shadow-lg scale-[1.02]" : "bg-white border-secondary hover:border-hover"}`}
                                  label={
                                    <div>
                                      <Heading4
                                      ...
                                      />
                                      {sub.description && (
                                        <p className={`text-[11px] mt-2 line-clamp-2 leading-relaxed transition-colors ${isActive ? "text-secondary/70" : "text-primary/50 group-hover:!text-hover/70"}`}>
                                          {sub.description}
                                        </p>
                                      )}
                                    </div>
                        ...
                        {/* Item Chips */}
                        <div className="mx-8 flex flex-wrap gap-2 mb-12 bg-secondary/30 p-4 rounded-lg">
                          {activeItems.map((item) => {
                            const isActive = activeItem?._id === item._id;
                            const buttonLabel = item.heading || item.title || item.blocks?.find((b) => b.block_type === "heading")?.title || "Untitled Option";

                            return (
                              <PrimaryButton
                                key={item._id}
                                onClick={() => setActiveItemMap(prev => ({ ...prev, [cat._id]: item }))}
                                className={`uppercase transition-all ${isActive ? "bg-primary text-secondary shadow-lg" : "bg-white !text-primary/60 hover:!text-secondary"}`}
                                label={buttonLabel}
                              />
                            );
                          })}
                        ...
                        <AnimatePresence mode="wait">
                          {activeItem && (
                            <motion.div
                              ...
                              className="mx-8 mb-8 bg-secondary/50 backdrop-blur-sm rounded-lg p-6 lg:p-12 border border-white shadow-xl overflow-hidden"
                            >
                              <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${isEven ? "" : "lg:flex-row-reverse"}`}>
                                ...
                                <div className={`space-y-8 ${isEven ? "lg:order-2" : "lg:order-1"}`}>
                                  ...
                                  <div className="space-y-4">
                                    {activeItem.description?.map((desc, i) => (
                                      ...
                                      >
                                        ...
                                        <RichParagraph className="italic">{desc}</RichParagraph>
                                      </motion.div>
                                    ))}
                                  </div>
                                  ...
                                  {activeItem.link && (
                                    <div className="pt-6">
                                      <Link href={activeItem.link} target="_blank" className="inline-block">
                                        <button className="group flex items-center gap-6 bg-primary text-secondary pl-8 pr-3 py-3 rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-[#001F3D]/90 transition-all shadow-lg hover:shadow-xl">
                                          {activeItem.title?.toLowerCase().includes("flooring") ? "view flooring options" : "view stain options"}
