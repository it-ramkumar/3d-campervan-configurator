...
export default function ClassicImageGrid({ images, layout = "standard" }) {
  const imageStyle = "w-full object-cover rounded-lg bbv-amber-line shadow-md hover:shadow-xl transition-shadow duration-300";

  if (layout === "vermont") {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-2 bg-primary/40 rounded-lg">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-1 lg:grid-rows-3 gap-4 h-160">
          ...
          <ImageWithSkeleton src={images[4]} alt="Vermont 5 campervan" className={`${imageStyle} h-48 md:h-52 col-span-2 lg:col-span-1 border-hover`} />
        </div>
      </div>
    );
  }
  ...
  if (layout === "motovan") {
    return (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-primary/60 rounded-lg">
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-3 bg-primary/40 rounded-lg border border-hover/20">
