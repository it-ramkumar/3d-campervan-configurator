...
const handleViewVans = () => {
  // 1. Define the internal categories you want to ignore
  ...
  // 2. Filter out the ignored items
  const filteredCategories = van?.category
    ? van.category.filter(cat => !ignoredCategories.includes(cat))
    : [];

  // 3. Check if we have vehicle categories left
  if (filteredCategories.length > 0) {
    const queryString = filteredCategories
      .map(cat => `category=${encodeURIComponent(cat)}`)
      .join('&');

    router.push(`/van-layouts?${queryString}`);
  }
  // 4. Fallback: If ONLY ignored categories exist, fall back to searching by title
  else if (van?.van_listing?.title) {
    // Slugs like "santa-monica-campervan" often map to a simpler title name like "Santa Monica"
    // We clean up or use the raw title to search
    const searchQuery = encodeURIComponent(van?.van_listing?.title.toLowerCase().trim());

    router.push(`/van-layouts?search=${searchQuery}`);
  }
  else {
    console.log("No valid categories or title found to filter by.");
  }
};
        ...
        <div className="max-w-[1440px] mx-auto pt-10 pb-20 px-6 md:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            <div className="lg:col-span-7">
              <VanGallery gallery={van.rendering} title={van?.van_listing?.title} />
            </div>
            {/* Info Panel */}
            <div className="lg:col-span-5 lg:sticky lg:top-24 h-fit space-y-8">
              ...
              <div className="flex flex-col gap-4 pt-4">
                ...
                />
    <button
    onClick={handleViewVans}
    className="btn-primary"
    // Optional: disable button if no valid categories are found
    // disabled={!van?.category || van.category.filter(cat => ![
    //   'portfolio-of-custom-builds',
    //   'layouts-for-solo-and-couple-travelers',
    //   'layouts-for-families-3-9-people'
    // ].includes(cat)).length === 0}
  >
    View Associated Vans
  </button>
              </div>
            </div>
          </div>
