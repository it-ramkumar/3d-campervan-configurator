      ...
      <div className="max-w-7xl mx-auto  px-6">
        ...
        <div className="flex flex-col md:flex-row items-end justify-between gap-8 mb-16">
          <div className="max-w-2xl">
            <WatermarkText text={"Ready for adventure"} className="uppercase mb-4"/>
          ...
          <div className="grid grid-cols-2 gap-3 w-full md:w-auto">
            {FEATURES.map((feat, idx) => (
              <div key={idx} className="flex items-center gap-3 bg-secondary/50 backdrop-blur-sm p-3 border border-secondary/20" style={{ borderRadius: 'var(--radius-md)' }}>
                <div className="text-primary bg-secondary p-2 shadow-sm rounded-lg">{feat.icon}</div>
        ...
        {filteredVans && filteredVans.length > 0 ? (
                ...
                <Link href={`/van-detail/${van.slug}`} className="relative block aspect-[16/10] overflow-hidden bg-slate-100">
                  ...
                  {van.gallery?.[0] ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-secondary-200 to-secondary-300">
                      ...
                      <WatermarkText text={"Image Coming Soon"} className=" uppercase opacity-40"/>
                  ...
                  {/* SOLD Badge */}
                  {van?.status === "sold" && (
                    <div className="absolute top-4 left-4 z-20 bg-red-600 text-secondary text-[10px] font-black px-4 py-1.5 uppercase tracking-widest shadow-lg" style={{ borderRadius: 'var(--radius-md)' }}>
                      Sold Out
                    </div>
                  )}
                  ...
                  <div className="absolute bottom-6 right-6 z-20 translate-y-12 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                    <div className="bg-secondary text-[var(--color-primary)] p-4 shadow-xl" style={{ borderRadius: 'var(--radius-md)' }}>
          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[1, 2].map((_, idx) => (
                <div key={idx} className="bg-secondary/40 border-2 border-dashed border-secondary-300 p-10 flex flex-col items-center text-center justify-center space-y-4" style={{ borderRadius: 'var(--radius-md)' }}>
                  ...
                  <div className="h-3 w-1/2 bg-secondary-100 rounded animate-pulse" />
                </div>
              ))}
        ...
        <div className="mt-20 flex flex-col items-center">
          <div className="w-20 h-[1px] bg-primary/20 mb-8" />
          <SecondaryButton
