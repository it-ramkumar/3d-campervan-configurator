...
export default function HeroSection() {
  ...
  // Pure JavaScript Title String formatting (TypeScript types removed)
  ...
  return (
<section className="relative w-full min-h-screen bg-primary text-secondary overflow-hidden grid grid-cols-1 lg:grid-cols-12 items-center px-8 md:px-16 gap-12 lg:gap-6 pt-24 lg:pt-0">
  ...
  {/* LEFT CONTENT — Typography & Actions (Takes 5 columns now instead of half) */}
  <div className={`relative z-10 flex flex-col lg:col-span-5 pr-0 lg:pr-6 transition-all duration-500 transform ${
    animating ? "opacity-0 translate-y-2" : "opacity-100 translate-y-0"
  }`}>

    {/* heading */}
    <h1 className="text-[clamp(40px,5.5vw,68px)] font-black italic uppercase leading-[1.0] tracking-tight mb-5">
      {renderTitle(activeSlide.title)}
    </h1>

    {/* accent line */}
    <div className="w-11 h-0.5 bg-hover rounded-full mb-5" />

    {/* description */}
    <p className="text-sm leading-[1.85] text-white/80 max-w-sm mb-9 min-h-[60px]">
      {activeSlide.desc}
    </p>
    ...
    <div className="inline-flex items-center gap-4 bg-white/[0.04] border border-white/[0.08] rounded-xl px-5 py-4 w-fit">
      ...
      <div>
        <div className="flex items-center gap-1 mb-0.5">
          {[...Array(5)].map((_, i) => (
            <svg key={i} className="w-3 h-3 text-[#D4A843]" fill="currentColor" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
          ))}
          <span className="text-white/80 text-[11px] font-semibold ml-1">5.0</span>
        </div>
        <p className="text-white/40 text-[10px] leading-tight">200+ happy customers</p>
      </div>
    </div>

  </div>

  {/* RIGHT CONTENT — Aesthetic Slider Display (Takes 7 columns now, giving image more space) */}
  <div className="relative w-full h-[55vh] lg:h-[85vh] lg:col-span-7 flex items-center justify-center z-10">
    {validSlides.map((slide, index) => {
      ...
      return (
        ...
        >
          {/* Removed max-width restrictions entirely and allowed full height utilization */}
          <div className="w-full h-full px-2 select-none flex items-center justify-center">
            <Image
              src={slide.image}
              alt={slide.title}
              width={1920}
  ...
  {/* Sleek Progress Dots Indicator */}
  <div className="absolute bottom-10 left-8 md:left-16 z-20 flex items-center gap-3">
    {validSlides.map((_, index) => (
      ...
      >
        <span className={`h-[3px] rounded-full transition-all duration-500 ease-out ${
          index === currentSlide
            ? "w-10 bg-[#D4A843]"
  ...
  <div className="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-[#080A0B] via-[#080A0B]/60 to-transparent pointer-events-none z-10" />
