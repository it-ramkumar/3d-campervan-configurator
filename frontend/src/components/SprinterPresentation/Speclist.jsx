...
import { ImageWithSkeleton } from "../Common/Common"; // Register ScrollTrigger
...
const packagesData = [
  {
    ...
    description: "Optional AWD package for enhanced traction. With \"Torque On Demand\" system, up to 50% axle power split.",
    features: [
  ...
  },
  {
    ...
    description: "Boosts driver/passenger comfort affordably. Ideal for daily commercial or personal use in Sprinter vans.",
    features: [
  ...
  },
  {
    ...
    description: "Enhances driver comfort and usability. Designed for efficiency during daily operations or long drives.",
    features: [
    ...
    image: "/sprinter/Rectangle 161 (2).jpg"
  },
  {
    id: "C03PKG",
    title: "C03PKG C03-Premium Plus Package",
    description: "Enhances technology and comfort for a premium driving experience. Tailored for drivers seeking advanced connectivity and safety features.",
    features: [
    ...
    image: "/sprinter/Rectangle 161 (3).jpg"
  },
  {
    id: "COSPKG",
    title: "COSPKG COS-Comfort Package (Seats)",
    description: "Upgrades seating for enhanced driver and passenger comfort. Focuses on ergonomic support for long drives or frequent use.",
    features: [
    ...
    image: "/sprinter/Rectangle 161 (4).jpg"
  },
  {
    id: "CA2PKG",
    title: "CA2PKG CA2-C02 Package (in Combination with C03)",
    description: "Enhances safety and comfort, designed to complement the C03-Premium Plus Package. Adds targeted features for improved driving experience when paired with C03's advanced tech.",
    features: [
  ...
  },
  {
    ...
    description: "Enhances the Sprinter's exterior aesthetics and functionality. Focuses on premium lighting and visibility for safety and style.",
    features: [
  ...
  },
  {
    ...
    description: "Enhances cabin aesthetics and functionality. Upgrades standard interior with premium materials for a refined look.",
    features: [
  ...
  },
  {
    ...
    description: "Enhances exterior aesthetics with premium chrome accents. Adds a polished, upscale look to the Sprinter's front grille.",
    features: [
...
const getFeatureIcon = (featureText, index) => {
  ...
  if (text.includes('maintains clearance') || text.includes('winter') || text.includes('off-road')) return <ClearanceIcon />;
  if (text.includes('turbodiesel') || text.includes('transmission')) return <TransmissionIcon />;

  // COOPKG - Comfort Package
  if (text.includes('lumbar support') || text.includes('comfort seats')) return <LumbarSupportIcon />;
  if (text.includes('control panel') || text.includes('overhead')) return <ControlPanelIcon />;
  if (text.includes('armrests')) return <ArmrestIcon />;
  if (text.includes('ergonomic') || text.includes('reduce fatigue')) return <ErgonomicsIcon />;
  ...
  if (text.includes('steering wheel') || text.includes('multifunction')) return <SteeringWheelIcon />;
  if (text.includes('storage compartment')) return <StorageIcon />;
  if (text.includes('camera') || text.includes('rearview')) return <CameraIcon />;

  // C03PKG - Premium Plus
  if (text.includes('touchscreen') || text.includes('mbux')) return <TouchscreenIcon />;
  ...
  if (text.includes('4-way lumbar') || text.includes('adjustable height')) return <LumbarSupportIcon />;
  if (text.includes('spine criteria') || text.includes('healthy spine')) return <SpineIcon />;
  ...
  if (text.includes('headlights') || text.includes('led high-performance')) return <HeadlightIcon />;
  ...
  if (text.includes('wall panels') || text.includes('interior trim')) return <InteriorIcon />;
  if (text.includes('upholstery') || text.includes('fabric')) return <UpholsteryIcon />;
  if (text.includes('professional appearance')) return <InteriorIcon />;
  if (text.includes('factory design') || text.includes('integrates')) return <IntegrationIcon />;

  // X67PKG - Chrome Package
  if (text.includes('chrome') || text.includes('radiator grille')) return <ChromeIcon />;
  if (text.includes('grille frame') || text.includes('body color')) return <GrilleIcon />;
...
const getAvailabilityIcon = (availabilityText, index) => {
  ...
  if (text.includes('models') || text.includes('cargo') || text.includes('vans')) return <VanModelIcon />;
  if (text.includes('$') || text.includes('add-on') || text.includes('price')) return <PriceIcon />;
  return <VanModelIcon />;
};

const getBenefitIcon = (benefitText, index) => {
  const text = benefitText.toLowerCase();
  if (text.includes('grip') || text.includes('snow') || text.includes('ice') || text.includes('terrain')) return <GripIcon />;
  if (text.includes('towing') || text.includes('stability')) return <TowingIcon />;
  if (text.includes('safety') || text.includes('monitoring') || text.includes('compliant')) return <SafetyIcon />;
  if (text.includes('cost') || text.includes('affordable') || text.includes('effective')) return <CostEffectiveIcon />;
  if (text.includes('comfort') || text.includes('fatigue') || text.includes('ergonomic')) return <ComfortIcon />;
  if (text.includes('integration') || text.includes('seamless')) return <IntegrationIcon />;
  return <SafetyIcon />;
};
...
export default function Speclist() {
  ...
  return (
    ...
    <section ref={containerRef} className="w-full bg-gradient-to-br from-gray-50 via-white to-gray-100 py-16 overflow-hidden">
      <div className="container mx-auto px-4 mb-12 text-center"> {/* Reduced margin */}
        <div className="inline-flex flex-col items-center mb-5"> {/* Reduced margin */}
          <div className="w-20 h-1 bg-gradient-to-r from-gray-800 via-gray-600 to-gray-800 rounded-full mb-2"></div> {/* Reduced size */}
        ...
        <h2 className="font-serif font-bold text-3xl md:text-4xl text-gray-900 leading-tight mb-4">
          Packages & Codes: <span className="text-gray-800">Decoding The Sprinter Spec List</span>
        </h2>
        <p className="text-base text-gray-600 max-w-xl mx-auto leading-relaxed"> {/* Reduced text size and max-width */}
      ...
      <div className="flex flex-col gap-16"> {/* REDUCED GAP */}
        {packagesData.map((pkg, index) => {
          ...
          const isBenefitsCollapsed = (pkg.benefits.length > collapsedItemCount) && !collapsedState[`${pkg.id}-benefits`];

          // Filter items based on collapsed state
          const displayedFeatures = isFeaturesCollapsed ? pkg.features.slice(0, collapsedItemCount) : pkg.features;
          const displayedAvailability = isAvailabilityCollapsed ? pkg.availability.slice(0, collapsedItemCount) : pkg.availability;
          const displayedBenefits = isBenefitsCollapsed ? pkg.benefits.slice(0, collapsedItemCount) : pkg.benefits;


          return (
            <div
              key={pkg.id}
              className={`spec-section relative container mx-auto px-4 md:px-8 flex flex-col lg:flex-row items-center gap-10 md:gap-16 ${ // REDUCED GAP
                !isEven ? "lg:flex-row-reverse" : ""
              }`}
            >
              {/* Background Elements (Reduced blur/size) */}
              <div className={`absolute -z-10 w-[300px] h-[300px] rounded-full blur-2xl opacity-5 bg-gray-800 ${
                isEven ? "-left-32 -top-24" : "-right-32 -top-24"
              }`}></div>
              <div className={`absolute -z-10 w-[250px] h-[250px] rounded-full blur-2xl opacity-5 bg-gray-700 ${
              ...
              <div className="w-full lg:w-1/2 text-gray-900 font-serif relative">
                ...
                <div className="inline-flex items-center px-3 py-2 bg-white/90 backdrop-blur-sm rounded-xl border border-gray-200 shadow-sm mb-6 group hover:shadow-md transition-all duration-300">
                  ...
                  <span className="font-mono text-xs font-semibold text-gray-700 tracking-wide">{pkg.id}</span>
                </div>
                ...
                <div className="relative mb-8">
                  ...
                  <p className="text-sm md:text-base leading-relaxed text-gray-700 pl-4 py-1.5 bg-gradient-to-r from-gray-50/80 to-gray-100/50 rounded-r-xl border-l-2 border-transparent">
                    {pkg.description}
                  </p>
                </div>
                ...
                <div className="space-y-6"> {/* Reduced space */}
                  {/* Key Features */}
                  <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-gray-200/50 hover:shadow-xl transition-all duration-500 hover:scale-[1.01]"> {/* Reduced padding/roundedness/scale */}
                    <div className="flex items-center gap-3 mb-4"> {/* Reduced gap/margin */}
                      <div className="w-10 h-10 bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg flex items-center justify-center shadow-md"> {/* Reduced size/roundedness/shadow */}
                    ...
                    <div className="space-y-3"> {/* Reduced space */}
                      {displayedFeatures.map((feature, i) => (
                        <div key={i} className="feature-item flex items-start gap-3 group p-2 rounded-lg hover:bg-gray-100/50 transition-all duration-300"> {/* Reduced padding/roundedness/gap */}
                          <div className="w-7 h-7 bg-white rounded-md flex items-center justify-center flex-shrink-0 shadow-sm border border-gray-200 group-hover:scale-110 transition-transform duration-300"> {/* Reduced size */}
                            {getFeatureIcon(feature, i)}
                          </div>
                          <span className="text-sm md:text-base text-gray-700 leading-relaxed pt-0.5">{feature}</span> {/* Reduced font size */}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-gray-200/50 hover:shadow-xl transition-all duration-500 hover:scale-[1.01]"> {/* Reduced padding/roundedness/scale */}
                    <div className="flex items-center gap-3 mb-4"> {/* Reduced gap/margin */}
                      <div className="w-10 h-10 bg-gradient-to-br from-gray-700 to-gray-800 rounded-lg flex items-center justify-center shadow-md"> {/* Reduced size/roundedness/shadow */}
      ...
      {/* Footer */}
      <div className="container mx-auto px-4 mt-16 md:mt-24 text-center"> {/* Reduced margin */}
        <div className="w-24 h-0.5 bg-gradient-to-r from-gray-300 via-gray-400 to-gray-300 rounded-full mx-auto mb-4"></div> {/* Reduced size */}
