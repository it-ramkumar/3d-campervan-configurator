...
const EmailManager = () => {
  ...
  const fetchEmails = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_REACT_APP_API_URL}/emails`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_REACT_APP_API_URL}/emails`, { email });
      if (res.data.success) {
  ...
  const deleteEmail = async (id) => {
    ...
    try {
      const res = await axios.delete(`${import.meta.env.VITE_REACT_APP_API_URL}/emails/${id}`);
      if (res.data.success) {
