...
export default function Navbar({ forceMobile }) {
  ...
  useEffect(() => {
    console.log("Page changed, closing ALL menus");
  ...
  // Simple mobile menu toggle - no animations
  ...
  // INSTANT menu close when clicking any link
  const handleMobileLinkClick = (e, link) => {
    e.preventDefault();
    console.log("Closing mobile menu instantly for:", link);
    
    // INSTANTLY close menu
    setIsMobileMenuOpen(false);
    setActiveMenu(null);
    
    // Force remove any body styles
    document.body.style.overflow = 'auto';
    document.body.style.position = 'static';
  ...
  return (
    <>
      ...
      {/* SIMPLE MOBILE MENU - No animations, just show/hide */}
      {isMobileMenuOpen && (
        <div
          ref={mobileMenuRef}
